...
-- First, let's check and fix the referral_commissions table structure
-- Ensure we have proper indexes and constraints
CREATE INDEX IF NOT EXISTS idx_referral_commissions_referrer_created ON public.referral_commissions(referrer_user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_referral_commissions_referred_created ON public.referral_commissions(referred_user_id, created_at DESC);

-- Add RLS policies for referral_commissions table
DROP POLICY IF EXISTS "Users can view their referral commissions" ON public.referral_commissions;
DROP POLICY IF EXISTS "Admin can manage all referral commissions" ON public.referral_commissions;
...
CREATE POLICY "Service can manage referral commissions" 
ON public.referral_commissions 
FOR ALL 
USING (true);

-- Create a function to process referral commissions at database level
...
CREATE OR REPLACE FUNCTION public.trigger_referral_commission_on_deposit()
RETURNS TRIGGER AS $$
BEGIN
  -- Only process when deposit is approved
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    PERFORM public.process_referral_commission(NEW.user_id, NEW.amount, 'deposit');
  END IF;
...
-- Create the trigger
DROP TRIGGER IF EXISTS referral_commission_deposit_trigger ON public.deposits;
CREATE TRIGGER referral_commission_deposit_trigger
  AFTER UPDATE ON public.deposits
  FOR EACH ROW
  EXECUTE FUNCTION public.trigger_referral_commission_on_deposit();
